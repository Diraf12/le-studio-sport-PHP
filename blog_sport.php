<?php 
    $link =  mysqli_connect("localhost", "farid", "" , "blog");
...
<?php 
    ...
    $results = mysqli_query($link, $sql);
    $numb_util = mysqli_num_rows($results);
    $row = mysqli_fetch_assoc($results);
    $key = array_keys($row);
?>
...
<?php endforeach; ?>
<?php echo "</table>" ?>
<?php echo "<h1 class='text-center bg-dark text-light'>il y a " .$numb_util . " utilisateurs dans la base de donées.</h1>"?>
